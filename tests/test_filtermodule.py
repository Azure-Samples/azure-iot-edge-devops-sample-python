...
class TestFilterModule(unittest.TestCase):
    ...
    def test_filter_more_than_threshold_alert_property_test(self):
        ...
        result = filter_message(source)
        self.assertEqual(
            "Alert", result.properties().get_internals()["MessageType"])
    ...
    def test_filter_more_then_threshold_copy_property(self):
        ...
        result = filter_message(source)
        self.assertEqual(expected, result.properties().get_internals()[
                         "customTestKey"])


def create_message(temperature):
    message_str = '{"machine":{"temperature":%s,"pressure":0}, "ambient":{"temperature":0,"humidity":0},"timeCreated":"%s"}' % (
